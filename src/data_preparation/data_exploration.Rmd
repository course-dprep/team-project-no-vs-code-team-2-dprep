...
This bar plot shows the average rating for each run time, divided in segments of 30 minutes. As we can see the average rating is at its highest for movies with a run time of 0-30 minutes, then drops at its lowest average at a run time of 60-90 minutes and from there the average rating increases again for each new segment of run time.
...
To see whether this difference in average rating differs per genre, we will first filter the data as such that we only use the 5 genres with the most amount of ratings on the IMDB website. These specific genres are:
...
This bar plot shows the average rating for each different run time for the five specific genres we chose. For all genres except Comedy the average rating is highest with a run time of 0-30 minutes, for Comedy the average rating is highest with a run time >180 minutes.
