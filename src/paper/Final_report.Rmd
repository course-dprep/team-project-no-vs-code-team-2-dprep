...
Looking back at our research question 'Is there a correlation between a movie's run time and its IMDb rating, with a focus on genre?', we can conclude that there is a very small positive correlation between the run time of a movies and its rating. However, diving deeper into this we found the optimal movie time to get a high rating on IMDB is between 0 and 30 minutes while movies with a run time larger than 180 minutes will score the second highest rating. Movies with a run time between 60 and 90 minutes will on average receive the lowest rating. 
